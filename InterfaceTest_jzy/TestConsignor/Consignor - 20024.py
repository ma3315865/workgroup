...
class Consignor20024(object):
    def __init__(self):
        """
        Consignor - 20024发布大订单

        """
        config = configparser.ConfigParser()
        config.read('D:\python\InterfaceTest_jzy\Config\config.ini')
        configer = con.TestFile(config)
        self.intface = post_interfaces.PostInterface('Consignor', '20024')
    ...
    def inter_datas(self):
        return {
            ...
            "params": {
                "id": "",  # total_order_id,指定不同意时可填写ID，可以重新编辑(可空)
                "customizeOrderNo": self.customize_order_no,  # 货主订单号
                "carrierCompanyId": "4fe3cc7c-d59d-11e8-bb03-00163e04a258",  # 承运商公司ID (可空)指定承运商是必填
                "projectName": self.project_name,  # 项目名称
                "loadName": self.load_name,  # 品名
                "loadWeight": "10",  # 货物总重量(吨)
                "loadVolume": "10",  # 货物总体积（立方米）
                "loadCount": "10",  # 货物件数 (可空)
                "singleSize": "单个长宽高 (可空)",  # 单个长宽高 (可空)
                "packageType": "木箱",  # 包装形式
                ...
                "sendLocationName": "黑龙江省佳木斯向阳",  # 发货地 省+市+县
                ...
                "receiveOrg": "收货单位test收货单位test",  # 收货单位~
                "receiveLocationPId": "31",  # 收货地省id
                "receiveLocationCId": "473",  # 收货地市id
                "receiveLocationAId": "3230",  # 收货地县区id
                "receiveLocationName": "新疆省克孜勒苏市阿克陶",  # 收货地 省+市+县
                "receiveDetailAddress": "收货详细地址test收货详细地址test",  # 收货详细地址
                ...
                "expectedLoadMoney": "11.00",  # 预期运费金额 保留两位小数点
...
if __name__ == '__main__':
    ...
    token.write_token()
    main = Consignor20024()
    print(main.interface_20024().text)